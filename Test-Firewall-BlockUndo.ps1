...
try {
    ...
    $result = [pscustomobject]@{
        Version='0.5.2-alpha'; Timestamp=(Get-Date).ToString('o')
    ...
    $result = [pscustomobject]@{ Version='0.5.2-alpha'; Timestamp=(Get-Date).ToString('o'); Passed=$false; Error=$_.Exception.Message }
} finally {
    try {
